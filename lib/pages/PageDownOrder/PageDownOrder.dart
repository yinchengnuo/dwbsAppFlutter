...
import 'package:dwbs_app_flutter/pages/PageMyOrder/components/OrderListItem.dart';

class PageDownOrder extends StatefulWidget {
  PageDownOrder({Key key}) : super(key: key);

  @override
  _PageDownOrderState createState() => _PageDownOrderState();
}

class _PageDownOrderState extends State<PageDownOrder> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  bool _loading = false;
  ...
  Map _pageData = {
    ...
    'page': [1, 1, 1, 1, 1],
    'data': [[], [], [], [], []],
    'requesting': [false, false, false, false, false],
    'navs': ['待审核', '待发货', '已发货', '已转单', '已完成'],
    'apis': [apiDownOrder1, apiDownOrder2, apiDownOrder3, apiDownOrder4, apiDownOrder5],
    'controllers': [ScrollController(), ScrollController(), ScrollController(), ScrollController(), ScrollController()],
  };
  ...
  void initState() {
    ...
    EventBus().on('DOWN_ORDER_LOADING', (res) {
      setState(() {
        this._loading = true;
      });
    });
    EventBus().on('DOWN_ORDER_HIDELOADING', (res) {
      setState(() {
        this._loading = false;
      });
    });
    EventBus().on('DELETE_ORDER', (index) {
      setState(() {
        this._pageData['data'][0].removeAt(index);
      });
    });
    EventBus().on('RECEIVE_MONEY', (index) {
      setState(() {
        if (this._pageData['page'][1] != 1) {
          this._pageData['data'][1].insert(0, Ycn.clone(this._pageData['data'][0][index]));
        }
        this._pageData['data'][0].removeAt(index);
      });
      this._scrollControllerListener(this._pageData['index']);
    });
    EventBus().on('SEND_ORDER', (index) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Loading(
      ...
      child: Scaffold(
        appBar: Ycn.appBar(context, title: '下级订单'),
        body: Column(
          children: <Widget>[
            Container(
                  ...
                  ...this._pageData['data'].map(
                    (item) {
                      if (this._pageData['page'][this._pageData['index']] == 1 &&
                        ...
                        return Center(child: Text('还没有订单呢'));
                      } else {
                        return RefreshIndicator(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) => OrderListItemWrapper(
                              ...
                              name: '下级订单' + this._pageData['navs'][this._pageData['data'].indexOf(item)],
