...
class PageIncomeRunning extends StatefulWidget {
  PageIncomeRunning({Key key}) : super(key: key);

  @override
  _PageIncomeRunningState createState() => _PageIncomeRunningState();
}

class _PageIncomeRunningState extends State<PageIncomeRunning> {
  Map _data;
  int _page = 1;
  ...
  void _request() async {
    if (!this._requesting && this._page != 0) {
      ...
      final res = (await apiIncomeRunningList({'page': this._page})).data;
      if (this._page == 1) {
        this._data = res['data'];
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: Ycn.appBar(context, title: '收入流水'),
