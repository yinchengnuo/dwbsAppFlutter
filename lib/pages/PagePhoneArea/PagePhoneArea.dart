...
class PagePhoneArea extends StatefulWidget {
  PagePhoneArea({Key key}) : super(key: key);

  @override
  _PagePhoneAreaState createState() => _PagePhoneAreaState();
}

class _PagePhoneAreaState extends State<PagePhoneArea> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: Ycn.appBar(context, title: '选择国家/地区'),
      body: ListView.builder(
        itemCount: list.length,
        itemBuilder: (BuildContext context, int index) => list[index]['list'].length > 0
            ? Container(
                padding: EdgeInsets.only(top: Ycn.px(8)),
                child: Column(
                  children: <Widget>[
                    Container(
                      height: Ycn.px(66),
                      color: Colors.white,
                      child: Row(children: <Widget>[SizedBox(width: Ycn.px(24)), Text(list[index]['name'])]),
                    ),
                    ...list[index]['list']
                        .map((item) => Container(
                              color: Colors.white,
                              margin: EdgeInsets.only(bottom: Ycn.px(2)),
                              height: Ycn.px(98),
                              child: MaterialInkWell(
                                onTap: () => Navigator.of(context).pop({'area': item['name'], 'code': item['code']}),
                                child: Container(
